...
func (p *Puppet) loginWithSharedSecret(mxid id.UserID) (string, error) {
	...
	req := mautrix.ReqLogin{
		...
		DeviceID:                 "QQ Bridge",
		InitialDeviceDisplayName: "QQ Bridge",
	}
...
func (br *QQBridge) newDoublePuppetClient(mxid id.UserID, accessToken string) (*mautrix.Client, error) {
...
func (p *Puppet) ProcessResponse(resp *mautrix.RespSync, _ string) error {
	if !p.customUser.IsLoggedIn() {
		p.log.Debugln("Skipping sync processing: custom user not connected to qq")
		return nil
	}
